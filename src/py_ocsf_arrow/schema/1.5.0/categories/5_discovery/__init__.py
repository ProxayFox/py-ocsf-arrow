...
def _load_inventory_info_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5001_inventory_info",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
    module = module_from_spec(spec)
    module.__package__ = "py_ocsf_arrow.schema.1.5.0.categories.5_discovery"
...
def _load_config_state_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5002_config_state",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_user_inventory_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5003_user_inventory",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_patch_state_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5004_patch_state",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_kernel_object_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5006_kernel_object_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_file_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5007_file_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_folder_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5008_folder_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_admin_group_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5009_admin_group_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_job_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5010_job_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_module_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5011_module_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_network_connection_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5012_network_connection_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_networks_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5013_networks_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_peripheral_device_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5014_peripheral_device_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_process_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5015_process_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_service_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5016_service_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_session_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5017_session_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_user_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5018_user_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_device_config_state_change_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5019_device_config_state_change",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_software_info_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5020_software_info",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_osint_inventory_info_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5021_osint_inventory_info",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_startup_item_query_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5022_startup_item_query",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_cloud_resources_inventory_info_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5023_cloud_resources_inventory_info",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_evidence_info_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.5_discovery._5040_evidence_info",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
