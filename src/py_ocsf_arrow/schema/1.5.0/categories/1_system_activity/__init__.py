...
def _load_file_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1001_file_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
    module = module_from_spec(spec)
    module.__package__ = "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity"
...
def _load_kernel_extension_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1002_kernel_extension_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_kernel_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1003_kernel_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_memory_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1004_memory_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_module_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1005_module_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_scheduled_job_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1006_scheduled_job_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_process_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1007_process_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_event_log_actvity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1008_event_log_actvity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_script_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.5.0.categories.1_system_activity._1009_script_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
