...
def _load_file_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1001_file_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
    module = module_from_spec(spec)
    module.__package__ = "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity"
...
@lru_cache(maxsize=1)
def _load_kernel_extension_activity_module() -> ModuleType:
    module_path = Path(__file__).with_name("1002_kernel_extension_activity.py")
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1002_kernel_extension_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def get_kernel_extension_activity_schema():
    """Return the generated Arrow schema for OCSF class 1002."""
    return (
        _load_kernel_extension_activity_module().get_kernel_extension_activity_schema()
    )


KERNEL_EXTENSION_ACTIVITY_SCHEMA = get_kernel_extension_activity_schema()
...
def _load_kernel_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1003_kernel_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_memory_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1004_memory_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_module_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1005_module_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_scheduled_job_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1006_scheduled_job_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_process_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1007_process_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
@lru_cache(maxsize=1)
def _load_event_log_actvity_module() -> ModuleType:
    module_path = Path(__file__).with_name("1008_event_log_actvity.py")
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1008_event_log_actvity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def get_event_log_actvity_schema():
    """Return the generated Arrow schema for OCSF class 1008."""
    return _load_event_log_actvity_module().get_event_log_actvity_schema()


EVENT_LOG_ACTVITY_SCHEMA = get_event_log_actvity_schema()


@lru_cache(maxsize=1)
def _load_script_activity_module() -> ModuleType:
    module_path = Path(__file__).with_name("1009_script_activity.py")
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1009_script_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def get_script_activity_schema():
    """Return the generated Arrow schema for OCSF class 1009."""
    return _load_script_activity_module().get_script_activity_schema()


SCRIPT_ACTIVITY_SCHEMA = get_script_activity_schema()


@lru_cache(maxsize=1)
def _load_peripheral_activity_module() -> ModuleType:
    module_path = Path(__file__).with_name("1010_peripheral_activity.py")
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.1_system_activity._1010_peripheral_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def get_peripheral_activity_schema():
    """Return the generated Arrow schema for OCSF class 1010."""
    return _load_peripheral_activity_module().get_peripheral_activity_schema()


PERIPHERAL_ACTIVITY_SCHEMA = get_peripheral_activity_schema()


__all__ = [
    "get_file_activity_schema",
    "FILE_ACTIVITY_SCHEMA",
    "get_kernel_extension_activity_schema",
    "KERNEL_EXTENSION_ACTIVITY_SCHEMA",
    ...
    "get_event_log_actvity_schema",
    "EVENT_LOG_ACTVITY_SCHEMA",
    "get_script_activity_schema",
    "SCRIPT_ACTIVITY_SCHEMA",
    "get_peripheral_activity_schema",
    "PERIPHERAL_ACTIVITY_SCHEMA",
]
