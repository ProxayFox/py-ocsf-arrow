...
def _load_security_finding_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2001_security_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
    module = module_from_spec(spec)
    module.__package__ = "py_ocsf_arrow.schema.1.7.0.categories.2_findings"
...
def _load_vulnerability_finding_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2002_vulnerability_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_compliance_finding_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2003_compliance_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_detection_finding_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2004_detection_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_incident_finding_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2005_incident_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
@lru_cache(maxsize=1)
def _load_data_security_finding_module() -> ModuleType:
    module_path = Path(__file__).with_name("2006_data_security_finding.py")
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2006_data_security_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def get_data_security_finding_schema():
    """Return the generated Arrow schema for OCSF class 2006."""
    return _load_data_security_finding_module().get_data_security_finding_schema()
...
@lru_cache(maxsize=1)
def _load_application_security_posture_finding_module() -> ModuleType:
    module_path = Path(__file__).with_name(
        "2007_application_security_posture_finding.py"
    )
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2007_application_security_posture_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def get_application_security_posture_finding_schema():
    """Return the generated Arrow schema for OCSF class 2007."""
    return _load_application_security_posture_finding_module().get_application_security_posture_finding_schema()


APPLICATION_SECURITY_POSTURE_FINDING_SCHEMA = (
    get_application_security_posture_finding_schema()
)


@lru_cache(maxsize=1)
def _load_iam_analysis_finding_module() -> ModuleType:
    module_path = Path(__file__).with_name("2008_iam_analysis_finding.py")
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.7.0.categories.2_findings._2008_iam_analysis_finding",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def get_iam_analysis_finding_schema():
    """Return the generated Arrow schema for OCSF class 2008."""
    return _load_iam_analysis_finding_module().get_iam_analysis_finding_schema()


IAM_ANALYSIS_FINDING_SCHEMA = get_iam_analysis_finding_schema()
...
__all__ = [
    ...
    "get_data_security_finding_schema",
    "DATA_SECURITY_FINDING_SCHEMA",
    "get_application_security_posture_finding_schema",
    "APPLICATION_SECURITY_POSTURE_FINDING_SCHEMA",
    "get_iam_analysis_finding_schema",
    "IAM_ANALYSIS_FINDING_SCHEMA",
]
