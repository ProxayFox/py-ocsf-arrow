...
def _load_network_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4001_network_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
    module = module_from_spec(spec)
    module.__package__ = "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity"
...
def _load_http_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4002_http_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_dns_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4003_dns_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_dhcp_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4004_dhcp_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_rdp_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4005_rdp_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_smb_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4006_smb_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_ssh_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4007_ssh_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_ftp_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4008_ftp_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_email_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4009_email_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_network_file_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4010_network_file_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_email_file_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4011_email_file_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_email_url_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4012_email_url_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_ntp_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4013_ntp_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
...
def _load_tunnel_activity_module() -> ModuleType:
    ...
    spec = spec_from_file_location(
        "py_ocsf_arrow.schema.1.8.0.categories.4_network_activity._4014_tunnel_activity",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load schema module from {module_path}")
